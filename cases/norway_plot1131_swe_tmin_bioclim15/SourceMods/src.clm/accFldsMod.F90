...
! #### Hui
    call init_accum_field (name='SWE_10', units='mm', &
         desc='average of snow water equivalent in October', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)
    call init_accum_field (name='TDA_5D', units='K', &
         desc='daily mean temperature May', &
...
! #### Hui
    call init_accum_field (name='PRECI01', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI02', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI03', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI04', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI05', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI06', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI07', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI08', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI09', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI10', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI11', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)

    call init_accum_field (name='PRECI12', units='MM H2O', &
         desc='January precipitation', &
         accum_type='runaccum', accum_period=not_used, &
         subgrid_type='pft', numlev=1, init_value=0._r8)
...
!
    ...
    real(r8), pointer :: h2osno(:)           ! snow water equivalent (mm H2O)
...
!#### Hui
    real(r8), pointer :: tmin_5(:)           ! Minimum temperature in May (Kelvin)
    real(r8), pointer :: tda_5d(:)           ! daily mean temperature in May (Kelvin)
    real(r8), pointer :: tmin_5_inst(:)      ! Instantaneous minimum temperature in May (Kelvin)
    real(r8), pointer :: preci01(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci02(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci03(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci04(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci05(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci06(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci07(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci08(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci09(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci10(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci11(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: preci12(:)           ! monthly mean precipitation (mm H2O)
    real(r8), pointer :: swe_10(:)           ! Snow water equivalent in October(mm)  
!#### Hui   
...
!#### Hui
    tmin_5           => clm3%g%l%c%p%pdgvs%tmin_5
    tda_5d           => clm3%g%l%c%p%pdgvs%tda_5d
    tmin_5_inst      => clm3%g%l%c%p%pdgvs%tmin_5_inst
    preci01           => clm3%g%l%c%p%pdgvs%preci01
    preci02           => clm3%g%l%c%p%pdgvs%preci02
    preci03           => clm3%g%l%c%p%pdgvs%preci03
    preci04           => clm3%g%l%c%p%pdgvs%preci04
    preci05           => clm3%g%l%c%p%pdgvs%preci05
    preci06           => clm3%g%l%c%p%pdgvs%preci06
    preci07           => clm3%g%l%c%p%pdgvs%preci07
    preci08           => clm3%g%l%c%p%pdgvs%preci08
    preci09           => clm3%g%l%c%p%pdgvs%preci09
    preci10           => clm3%g%l%c%p%pdgvs%preci10
    preci11           => clm3%g%l%c%p%pdgvs%preci11
    preci12           => clm3%g%l%c%p%pdgvs%preci12
    swe_10           => clm3%g%l%c%p%pdgvs%swe_10
...
!##### Hui
   if (month==5) then
      do p = begp,endp
         g = pgridcell(p)
         rbufslp(p) = forc_t(g)                       ! can be t_ref_2m?
      end do
      if (day>=2 .or. secs>= int(dtime)) then
        call update_accum_field  ('TDA_5D', rbufslp, nstep)
        call extract_accum_field ('TDA_5D', tda_5d, nstep)
        do p = begp,endp
          if (tda_5d(p) /= spval) then
!            print *, "tda_5d=",tda_5d(p)
            tmin_5_inst(p) = min(tda_5d(p), tmin_5_inst(p))
!            print *, "tmin_5_inst=", tmin_5_inst(p)
          endif
          if (day==31 .and. end_cd) then
            tmin_5(p)      = tmin_5_inst(p)
            tmin_5_inst(p) = spval
          endif
        end do
      end if
    ...
    do p = begp,endp
       c = pcolumn(p)
          if (month==1 .and. day==1 .and. secs==int(dtime)) then
             ...
             rbufslp(p) = 0._r8      ! keeps swe unchanged at other times
          end if
    end do
    call update_accum_field  ('SWE_10', rbufslp, nstep)
    call extract_accum_field ('SWE_10', swe_10, nstep)
   ...
   do p = begp,endp
       g = pgridcell(p)
          if (month==1 .and. day==1 .and. secs==int(dtime)) then
             ...
             rbufslp(p) = (forc_rain(g) + forc_snow(g))*dtime
          else
             rbufslp(p) = 0._r8      ! keeps swe unchanged at other times
          end if
    end do
    call update_accum_field  ('PRECI01', rbufslp, nstep)
    call extract_accum_field ('PRECI01', preci01, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI02', rbufslp, nstep)
    call extract_accum_field ('PRECI02', preci02, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI03', rbufslp, nstep)
    call extract_accum_field ('PRECI03', preci03, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI04', rbufslp, nstep)
    call extract_accum_field ('PRECI04', preci04, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI05', rbufslp, nstep)
    call extract_accum_field ('PRECI05', preci05, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI06', rbufslp, nstep)
    call extract_accum_field ('PRECI06', preci06, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI07', rbufslp, nstep)
    call extract_accum_field ('PRECI07', preci07, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI08', rbufslp, nstep)
    call extract_accum_field ('PRECI08', preci08, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI09', rbufslp, nstep)
    call extract_accum_field ('PRECI09', preci09, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI10', rbufslp, nstep)
    call extract_accum_field ('PRECI10', preci10, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI11', rbufslp, nstep)
    call extract_accum_field ('PRECI11', preci11, nstep)
   ...
   do p = begp,endp
    ...
    call update_accum_field  ('PRECI12', rbufslp, nstep)
    call extract_accum_field ('PRECI12', preci12, nstep)
...
!#### Hui
    call extract_accum_field ('TDA_5D', rbufslp, nstep)
    do p = begp,endp
       tda_5d(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI01', rbufslp, nstep)
    do p = begp,endp
       preci01(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI02', rbufslp, nstep)
    do p = begp,endp
       preci02(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI03', rbufslp, nstep)
    do p = begp,endp
       preci03(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI04', rbufslp, nstep)
    do p = begp,endp
       preci04(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI05', rbufslp, nstep)
    do p = begp,endp
       preci05(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI06', rbufslp, nstep)
    do p = begp,endp
       preci06(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI07', rbufslp, nstep)
    do p = begp,endp
       preci07(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI08', rbufslp, nstep)
    do p = begp,endp
       preci08(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI09', rbufslp, nstep)
    do p = begp,endp
       preci09(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI10', rbufslp, nstep)
    do p = begp,endp
       preci10(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI11', rbufslp, nstep)
    do p = begp,endp
       preci11(p) = rbufslp(p)
    end do

    call extract_accum_field ('PRECI12', rbufslp, nstep)
    do p = begp,endp
       preci12(p) = rbufslp(p)
    end do

    call extract_accum_field ('SWE_10', rbufslp, nstep)
    do p = begp,endp
       swe_10(p) = rbufslp(p)
    end do
