...
!#### Hui
   real(r8), pointer :: tmin_5_20(:)   !20-yr running mean of tmin_5
   real(r8), pointer :: swe_10_20(:)    !20-yr running mean of swe_10
   real(r8), pointer :: bioclim15_20(:)    !20-yr running mean of bioclim15
!#### Hui
...
!
! !LOCAL VARIABLES:
    integer  :: g,p,i                    ! indices
    real(r8) :: precmean,precstd
!-----------------------------------------------------------------------
...
!#### Hui
    tmin_5_20    => clm3%g%gdgvs%tmin_5_20
    swe_10_20    => clm3%g%gdgvs%swe_10_20
    bioclim15_20 => clm3%g%gdgvs%bioclim15_20
!#### Hui
...
!#### Hui
    ...
    do p = lbp,lbp             ! #### Hui, need to adjust, this only apply to single cell simulation
       g = pgridcell(p)
       precmean=(preci01(p)+preci02(p)+preci03(p)+preci04(p)+preci05(p)+preci06(p)+preci07(p)+preci08(p)+preci09(p)+preci10(p)+preci11(p)+preci12(p))/12
       precstd = sqrt(((preci01(p)-precmean)**2+(preci02(p)-precmean)**2+(preci03(p)-precmean)**2+(preci04(p)-precmean)**2+(preci05(p)-precmean)**2+(preci06(p)-precmean)**2+(preci07(p)-precmean)**2+(preci08(p)-precmean)**2+(preci09(p)-precmean)**2+(preci10(p)-precmean)**2+(preci11(p)-precmean)**2+(preci12(p)-precmean)**2)/11) 
       if (kyr == 2) then ! slevis: add ".and. start_type==arb_ic" here?
          ...
          tmin_5_20(g)= tmin_5(p)
          swe_10_20(g)= swe_10(p)/31/48
          bioclim15_20(g)= precstd/(1+precmean)*100
       end if
       tmomin20(g) = (19._r8 * tmomin20(g) + t_mo_min(p)) / 20._r8
       agdd20(g)   = (19._r8 * agdd20(g)   + agdd(p)    ) / 20._r8
       tmin_5_20(g) = (19._r8 * tmin_5_20(g) + tmin_5(p)) / 20._r8
       swe_10_20(g)   = (19._r8 * swe_10_20(g)   + swe_10(p)/31/48    ) / 20._r8
       bioclim15_20(g) = (19._r8 * bioclim15_20(g) + precstd/(1+precmean)*100) / 20._r8
      
       print *,  "tmomin2 =", tmomin20(g),t_mo_min(p) 
       print *, "tmin_5_20=", tmin_5_20(g), tmin_5(p)
       print *, "swe_10_20=", swe_10_20(g), swe_10(p)/31/48
       print *, "bioclim15_20=", bioclim15_20(g),precstd/(1+precmean)*100,preci01(p),preci02(p),preci03(p),preci04(p),preci05(p),preci06(p),preci07(p),preci08(p),preci09(p),preci10(p),preci11(p),preci12(p)
    end do
