    ...
    do p = lbp, ubp
       ...
       if (tmomin20(g) >= tcmin(ivt(p)) + SHR_CONST_TKFRZ ) then
          if (tmomin20(g) <= tcmax(ivt(p)) + SHR_CONST_TKFRZ  .and. agdd20(g) >= gddmin(ivt(p))) then
             estab(p) = .true.
!####Hui
             if (ivt(p)==2 .and. swe_10_20(g)>= 124/10 )then
                estab(p) = .false.
             else if (ivt(p)==2 .and. tmin_5_20(g)<=-5.0+SHR_CONST_TKFRZ )then
                estab(p) = .false.
!             else if (ivt(p)==2 .and. bioclim15_20(g)>=50 )then
!                estab(p) = .false.
             else if (ivt(p)==8 .and. swe_10_20(g)>= 186/10 )then
                estab(p) = .false.
             else if (ivt(p)==8 .and. tmin_5_20(g)<=-7.5+SHR_CONST_TKFRZ )then
                estab(p) = .false.
             else if (ivt(p)==11 .and. swe_10_20(g)>= 372/10)then
                estab(p) = .false.
             else if (ivt(p)==11 .and. tmin_5_20(g)<=-10.0+SHR_CONST_TKFRZ )then
                estab(p) = .false.
             else
                estab(p) = .true.
             end if
